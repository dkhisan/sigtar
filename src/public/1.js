...
/* harmony default export */ __webpack_exports__["default"] = ({
  ...
  methods: {
    fetchTask: function fetchTask(task) {
      ...
      _api.call('get', "/api/tasks/".concat(task)).then(function (res) {
        if (res.status === 200) {
          if (res.data.status !== 404) {
            _this.task_ = res.data;
            _this.response.success = true;
          } else {
            _this.response.error = res.data.message;
          }
        }
      }).catch(function (err) {
        if (err.response) {
          _this.response.status = err.response.data.status;
          _this.response.error = err.response.data.error;
        } else if (err.request) {
          _this.request.status = err.request.data.status;
          _this.request.error = err.request.data.error;
        }
      });
    },
    ...
    destroy: function destroy() {
      ...
      _api.call('delete', "/api/tasks/".concat(this.task_.id, "/remove")).then(function (res) {
        if (res.status === 200) {
          if (res.data.status === 422) {
            _this2.response.error = res.data.message;
          }
        } else if (res.status === 204) {
          _this2.response.success = true;
          _this2.response.message = res.data.message;
        }
      }).catch(function (err) {
        if (err.response) {
    ...
    submit: function submit() {
      ...
      _api.call('patch', "/api/tasks/".concat(this.task_.id, "/edit"), this.task_).then(function (res) {
        if (res.status === 200) {
          if (res.data.status !== 422) {
            _this3.response.success = true;
            _this3.response.message = res.data.message;
          } else {
            _this3.response.error = res.data.message;
          }
        }
      }).catch(function (err) {
        if (err.response) {
                            ...
                            _c("div", { staticClass: "col-12" }, [
                              _c("div", { staticClass: "form-group" }, [
                                ...
                                _c("textarea", {
                                  ...
                                  domProps: { value: _vm.task_.description },
                                  on: {
                                    input: function($event) {
                                      if ($event.target.composing) {
                                        return
...
var staticRenderFns = [
  function() {
    ...
    return _c("div", { staticClass: "modal-body" }, [
      _c("p", [
        _vm._v("Tem certeza que quer fazer isso? (Isso não pode ser desfeito)")
      ])
    ])
  },
  function() {
